...
import { Github, Linkedin, Mail, Terminal as TermIcon, Sparkles, Server } from 'lucide-react'
...
const MovingDotsBackground = () => {
  ...
  useEffect(() => {
    ...
    const DOT_COUNT = Math.min(Math.floor((canvas.width * canvas.height) / 11000), 75)
    ...
    const render = () => {
      ...
      dots.forEach((dot) => {
        ...
        const currentAlpha = dot.baseAlpha + Math.sin(dot.pulseAngle) * 0.05
...
const COMMANDS = {
  ...
  whoami: 'Muhammad Aleem Madni Khan — Software Engineering Student @ SMIU, Karachi.',
  skills: 'Backend: Spring Boot, REST APIs, Security, JWT | Frontend: React, Tailwind | DB: MySQL',
  focus: 'Building scalable full-stack applications & mastering backend systems architecture.',
  contact: 'Email: aleem90909m@gmail.com | GitHub: github.com/AleemMadniKhan',
}
...
function InteractiveTerminal() {
  ...
  const handleCommand = (e) => {
    ...
    if (trimmed === 'clear') {
      ...
      setHistory((prev) => [...prev, { cmd: trimmed, output: COMMANDS[trimmed] }])
    } else {
      setHistory((prev) => [
        ...prev,
        { cmd: trimmed, output: `Command not found: '${trimmed}'. Type 'help' for options.` },
      ])
    }
    setInput('')
  }
      ...
      <div className="flex items-center justify-between px-3 sm:px-4 py-3 border-b border-white/10 bg-bg-secondary/80">
        <div className="flex items-center gap-2">
          ...
          <span className="ml-1 sm:ml-2 text-slate-400 flex items-center gap-1.5 text-xs truncate">
            <TermIcon className="w-3.5 h-3.5 text-amber-200/70 shrink-0" /> aleem@dev:~
          </span>
        </div>
        <span className="text-[10px] text-slate-500 uppercase tracking-widest font-sans shrink-0">Terminal</span>
      </div>

      <div className="p-4 sm:p-5 h-[260px] sm:h-[280px] overflow-y-auto space-y-3 scrollbar-thin">
        <p className="text-slate-500 italic">
          Type <span className="text-amber-200/80 font-bold">'help'</span> to see available commands.
        </p>
        {history.map((item, idx) => (
          <div key={idx} className="space-y-1">
            ...
            <div className="text-slate-300 pl-4 border-l border-amber-500/30 break-words">{item.output}</div>
          </div>
        ))}
...
export default function Hero() {
  return (
    <section id="hero" className="relative min-h-screen flex items-center pt-28 sm:pt-32 pb-16 sm:pb-20 px-4 sm:px-6 overflow-hidden bg-bg-primary">
      ...
      <div className="relative max-w-7xl mx-auto grid lg:grid-cols-[1.1fr_0.9fr] gap-10 lg:gap-12 items-center w-full z-10">
        ...
        >
          <div className="section-eyebrow mb-6 flex items-center gap-2">
            ...
            <span>Software Engineer · Karachi, PK</span>
          </div>
          ...
          <p className="mt-2 text-sm sm:text-base text-slate-400 max-w-xl leading-relaxed">
            Crafting secure backend architectures and scalable digital experiences with{' '}
            <span className="text-slate-100 font-medium">Spring Boot</span>,{' '}
            <span className="text-slate-100 font-medium">React</span>, and modern database systems.
          </p>
