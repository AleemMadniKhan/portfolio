...
const COMMANDS = {
  ...
  whoami:
    "Muhammad Aleem Madni Khan — Software Engineering Student @ SMIU, Karachi.",
  skills:
    "Backend: Spring Boot, REST APIs, Security, JWT | Frontend: React, Tailwind | DB: MySQL",
  focus:
    "Building scalable full-stack applications & mastering backend systems architecture.",
...
function InteractiveTerminal() {
  ...
  const handleCommand = (e) => {
    ...
    if (trimmed === "clear") {
      ...
      setHistory((prev) => [
        ...prev,
        { cmd: trimmed, output: COMMANDS[trimmed] },
      ]);
    } else {
      setHistory((prev) => [
        ...prev,
    ...
    <div className="glass rounded-2xl shadow-2xl overflow-hidden font-mono text-xs sm:text-sm border border-white/10 w-full max-w-full">
      ...
      <div className="p-4 sm:p-5 h-[260px] sm:h-[280px] overflow-y-auto space-y-3 scrollbar-thin">
        <p className="text-slate-500 italic">
          Type <span className="text-amber-200/80 font-bold">'help'</span> to
      ...
      <div className="relative max-w-7xl mx-auto grid lg:grid-cols-[1.1fr_0.9fr] gap-10 lg:gap-12 items-center w-full z-10">
        ...
        >
          ...
          <p className="mt-2 text-sm sm:text-base text-slate-400 max-w-xl leading-relaxed">
            Crafting secure backend architectures and scalable digital
            experiences with{" "}
            <span className="text-slate-100 font-medium">Spring Boot</span>,{" "}
            <span className="text-slate-100 font-medium">React</span>, and
